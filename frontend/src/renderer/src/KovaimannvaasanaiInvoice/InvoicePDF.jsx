...
const InvoiceDocument = ({ selectedInvoice, profile }) => {
  ...
  useEffect(() => {
    if (profile?.logo) {
      getBase64Image(`${backendUrl}/uploads/${profile.logo}`)
        ...
        <View style={styles.table}>
          <View style={styles.tableHeader}>
            {[
              'S.NO',
              'Description',
              `Boxes   ${''}(${selectedInvoice?.computedtotals?.totalBoxQty})`,
              'Kgs',
              'Rate',
              'GST',
              'Amount'
          ...
          {currentItems.map((item, index) => {
            ...
            return (
              <View key={index} style={styles.tableRow}>
                ...
                <Text style={{ ...styles.cell, width: columnWidths[2] }}>{item.box || '-'}</Text>
                <Text style={{ ...styles.cell, width: columnWidths[3] }}>
                  {`${(Number(item.kilo || 0) + Number(item.grams || 0) / 1000).toFixed(3).replace(/\.?0+$/, '')} kg`}
                </Text>
            ...
            <View style={styles.footerBoxed}>
              ...
              <View style={styles.summaryBox}>
                {[
                  ...
                  ['GST on BC & DC :', selectedInvoice?.computedtotals?.bc_dc_gst],
