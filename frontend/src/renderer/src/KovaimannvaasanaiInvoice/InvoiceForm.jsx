...
const InvoiceForm = () => {
  ...
  const [boxCharge, setBoxCharge] = useState(() => {
    return localStorage.getItem('boxCharge') || ''
  })
  useEffect(() => {
    if (nextInvoiceNo) {
  ...
  const handleSubmit = async (values) => {
    ...
    if (Number(deliveryCharge) === 0 && Number(boxCharge) === 0) {
      Modal.confirm({
        title: 'Delivery Charge and Per Box Charge are 0',
    ...
    >
      ...
      >
        ...
        <Row gutter={16}>
          ...
          <Col span={3}>
            <Form.Item label="Per Box Charge">
